...
<div class="max-w-screen-lg mx-auto p-6 space-y-4 bg-gray-100">
    ...
    <div class="flex justify-end mb-4">
      ...
      <table class="min-w-full table-auto border-collapse">
        ...
        <tbody>
          <!-- Loop through product groups from the database -->
          @foreach($groups as $group)
          <tr class="{{ $loop->even ? 'bg-gray-100' : 'bg-white' }}">
            <td class="border px-4 py-2">{{ $group->MenuGr_id }}</td>
            <td class="border px-4 py-2">{{ $group->MenuGr_Khname }}</td>
            <td class="border px-4 py-2">{{ $group->MenuGr_Engname }}</td>
